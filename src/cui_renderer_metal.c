...
_cui_renderer_metal_create(CuiArena *temporary_memory, id<MTLDevice> metal_device)
{
    ...
    if (renderer->command_queue)
    {
        ...
        "\n"
        "    constant TexturedRect *textured_rect = (constant TexturedRect *) (push_buffer + rect_offset);\n"
        "\n"
        "    short x0 = textured_rect->position_min.x;\n"
        "    short y0 = textured_rect->position_min.y;\n"
        "    short x1 = textured_rect->position_max.x;\n"
        "    short y1 = textured_rect->position_max.y;\n"
        "\n"
        "    float2 uv1 = float2((float) textured_rect->uv_min.x, (float) textured_rect->uv_min.y);\n"
        "    float2 uv2 = float2((float) textured_rect->uv_max.x, (float) textured_rect->uv_min.y);\n"
        "    float2 uv3 = float2((float) textured_rect->uv_max.x, (float) textured_rect->uv_max.y);\n"
        "    float2 uv4 = float2((float) textured_rect->uv_min.x, (float) textured_rect->uv_max.y);\n"
        "\n"
        "    if (textured_rect->clip_rect)\n"
        "    {\n"
        "        constant ClipRect *clip_rect = (constant ClipRect *) (push_buffer + (textured_rect->clip_rect - 1));\n"
        "\n"
        "        if (x0 < clip_rect->min.x)\n"
        "        {\n"
        "            float t = (float) (clip_rect->min.x - x0) / (float) (x1 - x0);\n"
        "            x0 = clip_rect->min.x;\n"
        "            uv1 = mix(uv1, uv2, t);\n"
        "            uv4 = mix(uv4, uv3, t);\n"
        "        }\n"
        ...
        "        {\n"
        "            float t = (float) (clip_rect->min.y - y0) / (float) (y1 - y0);\n"
        "            y0 = clip_rect->min.y;\n"
        "            uv1 = mix(uv1, uv4, t);\n"
        "            uv2 = mix(uv2, uv3, t);\n"
        "        }\n"
        ...
        "        {\n"
        "            float t = (float) (clip_rect->max.x - x0) / (float) (x1 - x0);\n"
        "            x1 = clip_rect->max.x;\n"
        "            uv2 = mix(uv1, uv2, t);\n"
        "            uv3 = mix(uv4, uv3, t);\n"
        "        }\n"
        ...
        "        {\n"
        "            float t = (float) (clip_rect->max.y - y0) / (float) (y1 - y0);\n"
        "            y1 = clip_rect->max.y;\n"
        "            uv4 = mix(uv1, uv4, t);\n"
        "            uv3 = mix(uv2, uv3, t);\n"
        "        }\n"
        ...
        "    result.color         = textured_rect->color;\n"
        "    result.uv            = uv;\n"
        "    result.texture_index = textured_rect->texture_index;\n"
        "\n"
        "    return result;\n"
        "}\n"
        "\n"
